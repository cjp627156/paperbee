...
export class ArticleHomeComponent implements OnInit {
  ...
  mouth = false;
  orderBy = [];
  order = "ARTICLE_VIEWS DESC, CREATION_DATE DESC";
  ...
  */
  ...
  articlesQuery() {
    let param = {
      ...
      "label": this.label,
      "orderBy": this.order == "" ? "ARTICLE_VIEWS DESC, ARTICLE_VIEWS DESC, CREATION_DATE DESC" : this.order,
      "page": this.pageIndex,
  ...
  detail(data) {
    this.articleService.addArticleView(data.id).subscribe(x => {
      data.articleViews+=1;
  ...
  getMore() {
    ...
    let param = {
      "searchContent": this.searchContent,
      "label": this.label,
      "orderBy": this.order == "" ? "ARTICLE_LIKES DESC, ARTICLE_VIEWS DESC, CREATION_DATE DESC" : this.order,
      "page": this.pageIndex,
      "pageSize": this.pageSize
    };
    this.articleService.getArticles(param).subscribe(x => {
